...
/* ══ TTS — ElevenLabs via Worker ═════════ */
let _ttsAudio=null; // current Audio object
let _ttsAbort=false;

async function speak(text,onDone){
  if(!SETTINGS.voiceEnabled){onDone?.();return}
  if(CONFIG.workerURL.includes('YOUR_')){
    // Fallback: Web Speech API ถ้ายังไม่ตั้งค่า Worker
    _speakFallback(text,onDone);return;
  }
  try{
    // Stop previous audio
    if(_ttsAudio){_ttsAudio.pause();_ttsAudio=null}
    const res=await fetch(`${CONFIG.workerURL}/tts`,{
      method:'POST',
    ...
    audio.onerror=()=>{URL.revokeObjectURL(url);_ttsAudio=null;onDone?.()};
    await audio.play();
  }catch(e){
    console.warn('TTS error:',e.message);
    // fallback to Web Speech
    _speakFallback(text,onDone);
  }
}

function _speakFallback(text,onDone){
  if(!window.speechSynthesis){onDone?.();return}
  const u=new SpeechSynthesisUtterance(text);
  u.lang='th-TH';u.volume=SETTINGS.voiceVolume;u.rate=0.9;
...
function speakSequence(lines,onAllDone){
  ...
  let i=0;
  function next(){
    if(_ttsAbort||i>=lines.length){onAllDone?.();return}
    const line=lines[i++];
    speak(line.text,()=>{
      if(_ttsAbort){onAllDone?.();return}
...
function pauseGame(){closeMenu();S.paused=true;stopCountdown();stopSpeakSequence();$('pauseOverlay')?.classList.add('show')}
...
function doEndGame(){
  ...
  closeNightScreens();stopCountdown();stopSpeakSequence();
